...
interface FibonacciResultsProps {
  results: FibonacciOptimizationResult[];
  onSelectConfiguration: (result: FibonacciOptimizationResult) => void;
}

export function FibonacciResults({ results, onSelectConfiguration }: FibonacciResultsProps) {
  const [showAll, setShowAll] = useState(false);
  const [sortBy, setSortBy] = useState<keyof FibonacciOptimizationResult>('score');
  ...
  const handleSort = (column: keyof FibonacciOptimizationResult) => {
    if (sortBy === column) {
    ...
    <div className="bg-white rounded-xl shadow-lg p-6">
      <div className="flex items-center justify-between mb-6">
        <h3 className="text-xl font-bold text-gray-900 flex items-center">
          ...
          Fibonacci Scalping Optimization Results
        </h3>
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
            {sortedResults.slice(0, 3).map((result, index) => (
              <div
                key={`${result.swingLookback}-${result.goldenZoneMin}-${result.profitTarget}-${result.leverage}`}
                className={`rounded-lg p-4 cursor-pointer transition-all hover:shadow-md ${
                  index === 0 
                    ...
                    : 'bg-gradient-to-br from-pink-50 to-purple-100 border-2 border-pink-300'
                }`}
                onClick={() => onSelectConfiguration(result)}
              >
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center">
                    {index === 0 && <Trophy className="h-5 w-5 text-purple-600 mr-1" />}
                ...
                <div className="space-y-1 mb-3">
                  <p className="text-xs text-gray-600">
                    Swing({result.swingLookback}) GZ({result.goldenZoneMin}-{result.goldenZoneMax})
                  </p>
                  <p className="text-xs text-gray-600">
                    MinSwing({(result.minSwingSize*100).toFixed(1)}%) Vol({result.volumeThreshold})
                  </p>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  ...
                  <div>
                    ...
                    <span className="ml-1 font-medium">{result.riskRewardRatio.toFixed(1)}:1</span>
                  </div>
                </div>
              </div>
            ))}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="text-left py-2 px-3 font-medium text-gray-600">Avg Trade</th>
                  <th 
                    className="text-left py-2 px-3 font-medium text-gray-600 cursor-pointer hover:text-gray-900"
                ...
                {displayResults.map((result, index) => (
                  ...
                  >
                    <td className="py-2 px-3">
                      <div className="flex items-center">
                        {index === 0 && <Trophy className="h-4 w-4 text-purple-500 mr-1" />}
                    ...
                    <td className="py-2 px-3 text-xs">
                      <div className="space-y-1">
                        ...
                        <div>MinSwing: {(result.minSwingSize*100).toFixed(1)}%</div>
                      </div>
                    </td>
                    <td className="py-2 px-3 text-xs">
                      <div className="space-y-1">
                        <div>{formatPercentage(result.profitTarget)}/{formatPercentage(result.stopLossPercent)}</div>
                        <div className="text-purple-600">{result.leverage}x | {result.maxHoldingMinutes}m</div>
                        <div>R/R: {result.riskRewardRatio.toFixed(1)}:1</div>
                      </div>
                    </td>
                    <td className="py-2 px-3 text-xs">
                      <div className="space-y-1">
                        <div>{result.averageTradeMinutes?.toFixed(1)}m</div>
